...
class TelegramConfig(Base):
    ...
    react_emoji: str = "👀"
    group_policy: Literal["mention", "ambient", "open"] = "mention"
...
class TelegramChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content.strip() == "__SILENT__":
            logger.debug("Telegram ambient mode - LLM chose not to respond")
            return
    ...
    def _build_message_metadata(message, user) -> dict:
        ...
        thread_type = "User" if message.chat.type == "private" else "Group"
        return {
    ...
    async def _is_group_message_addressed_to_bot(self, message) -> bool:
        """Return True when a group message explicitly mentions or replies to the bot."""
    ...
    async def _group_message_route(self, message) -> Literal["process", "ambient", "ignore"]:
        """Route Telegram group messages according to group_policy."""
        if message.chat.type == "private" or self.config.group_policy == "open":
            return "process"
        if await self._is_group_message_addressed_to_bot(message):
            return "process"
        if self.config.group_policy == "ambient":
            return "ambient"
        return "ignore"

    async def _is_group_message_for_bot(self, message) -> bool:
        """Allow group messages when policy is open, ambient, @mentioned, or replying to the bot."""
        return await self._group_message_route(message) != "ignore"
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        route = await self._group_message_route(message)
        if route == "ignore":
            return
        telegram_ambient = route == "ambient"
        ...
        metadata = self._build_message_metadata(message, user)
        metadata["group_ambient"] = telegram_ambient
        if telegram_ambient:
            ambient_sender = metadata.get("sender_name") or sender_id
            content = (
                f"[GROUP AMBIENT - you were NOT @mentioned. "
                f"The following is a Telegram group message from {ambient_sender}. "
