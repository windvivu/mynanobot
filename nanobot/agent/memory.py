...
class MemoryStore:
    ...
    _DREAM_REPLAY_THRESHOLD = 50  # migrations with <= this many entries let Dream process them
    ...
    def _maybe_migrate_legacy_history(self) -> None:
        ...
        if self.legacy_history_file.stat().st_size == 0:
            try:
                self.legacy_history_file.unlink()
                logger.debug("Removed empty legacy HISTORY.md (nothing to migrate)")
            except OSError:
                pass
            return
        ...
        try:
            if entries:
                ...
                # Only mark as "already processed" when history is large enough
                # that replaying would incur significant API cost.  For small
                # histories, let Dream process them — the cost is negligible
                # and the memory retention benefit is high.
                if len(entries) > self._DREAM_REPLAY_THRESHOLD:
                    self._dream_cursor_file.write_text(str(last_cursor), encoding="utf-8")
                # else: dream_cursor file not created → defaults to 0 → Dream will process
...
class Dream:
    ...
    def _annotate_with_ages(self, content: str) -> str:
        ...
        Returns the original content unchanged if git is unavailable or fails.
        """
