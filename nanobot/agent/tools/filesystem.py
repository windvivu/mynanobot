...
_OFFICE_DOCUMENT_EXTENSIONS = frozenset({".docx", ".xlsx", ".pptx"})


def _blocked_device_candidates(path: str | Path) -> set[str]:
    candidates = {str(path)}
    ...
    try:
        if p.is_symlink():
            link = p.readlink()
            candidates.add(str(link))
            candidates.add(link.as_posix())
            if not link.is_absolute():
                resolved_link = (p.parent / link).resolve(strict=False)
                candidates.add(str(resolved_link))
                candidates.add(resolved_link.as_posix())
    except OSError:
        pass
    return candidates
...
def _is_blocked_device(path: str | Path) -> bool:
    ...
    for raw in _blocked_device_candidates(path):
...
class ReadFileTool(_FsTool):
    ...
    def description(self) -> str:
        return (
            "Read a file (text, image, or supported office document). Text output format: LINE_NUM|CONTENT. "
            "Images return visual content for analysis. DOCX/XLSX/PPTX files are extracted to text. "
    ...
    async def execute(self, path: str | None = None, offset: int = 1, limit: int | None = None, pages: str | None = None, **kwargs: Any) -> Any:
        try:
            ...
            text_content = self._extract_text_content(fp, raw, path, mime)
            if text_content.startswith("Error:"):
                return text_content
            if not text_content:
                return f"(Empty file: {path})"

            all_lines = text_content.replace("\r\n", "\n").splitlines()
            total = len(all_lines)
    ...
    def _extract_text_content(self, fp: Path, raw: bytes, path: str, mime: str | None) -> str:
        if fp.suffix.lower() in _OFFICE_DOCUMENT_EXTENSIONS:
            extracted = extract_text(fp)
            if extracted is None:
                return (
                    f"Error: Cannot read binary file {path} (MIME: {mime or 'unknown'}). "
                    "Only UTF-8 text, images, and supported office documents are supported."
                )
            if extracted.startswith("[error:"):
                return f"Error: Failed to extract text from {path}: {extracted}"
            return extracted
