...
class AgentRunner:
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        for iteration in range(spec.max_iterations):
            try:
                ...
                # Snipping may create new orphans; clean up before sending to model.
            ...
            context = AgentHookContext(
                ...
                progress_emitter=spec.progress_callback,
            )
    ...
    def _append_model_error_placeholder(messages: list[dict[str, Any]]) -> None:
        """Append a placeholder assistant message after a model error.

        Keeps the conversation history structurally valid (no trailing tool
        messages without a following assistant turn) so the next LLM call is
        not rejected due to an invalid role sequence.
        """
    ...
    def _drop_orphan_tool_results(
        ...
        This can happen when _snip_history() cuts the assistant message that
        originated a tool call but keeps the result.  Feeding orphan results to
        the model triggers provider errors (e.g. OpenAI 400, Anthropic 400).
        """
